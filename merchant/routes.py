...
def predict():
    ...
    try:
        ...
        response["final_prediction"] = int(result.get("final_prediction", int(float(result.get("raw_probability", 0.0)) >= 0.5)))
        response["confidence"] = float(result.get("confidence", result.get("raw_probability", 0.0)))
        response["model_outputs"] = dict(result.get("model_outputs", {}))
...
def _predict_transaction(payload: dict[str, Any], bundle=None) -> dict[str, Any]:
    ...
    if not isinstance(result.get("models"), dict):
        legacy_scores = result.get("model_scores", {}) if isinstance(result.get("model_scores"), dict) else {}
        normalized_models: dict[str, float] = {}
        for name, value in legacy_scores.items():
            try:
                score = float(value)
            except Exception:
                continue
            normalized_models[str(name)] = max(0.0, min(1.0, score / 100.0 if score > 1.0 else score))
        if "ensemble" not in normalized_models:
            normalized_models["ensemble"] = float(result.get("raw_probability", 0.0) or 0.0)
        result["models"] = normalized_models

    if not isinstance(result.get("model_outputs"), dict):
        models = result.get("models", {}) if isinstance(result.get("models"), dict) else {}
        result["model_outputs"] = {
            "xgboost": float(models.get("xgboost", 0.0) or 0.0),
            "lightgbm": float(models.get("lightgbm", 0.0) or 0.0),
            "ann": float(models.get("ann", models.get("dnn", 0.0)) or 0.0),
            "transformer": float(models.get("transformer", 0.0) or 0.0),
        }
    if "confidence" not in result:
        result["confidence"] = float(result.get("raw_probability", 0.0) or 0.0)
    if "final_prediction" not in result:
        result["final_prediction"] = int(float(result.get("confidence", 0.0)) >= 0.5)
